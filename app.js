 ...
 */
function selectClinic(lat, lon, name, type, addr, clinicObj) {
...
function findAndRouteToNearestClinic(userLat, userLon) {
    ...
    if (nearest) {
        ...
        selectClinic(nearest.lat, nearest.lon, nearest.name, nearest.type, nearest.tags.addr || nearest.tags['addr:full'] || '', nearest);
...
function handlePosition(position) {
    ...
    // If accuracy is poor (>100m) treat as likely IP-based / low-confidence and
...
function startAutoGeolocation() {
    ...
    liveTrackingWatchId = navigator.geolocation.watchPosition(
        (pos) => {
            ...
            // Warn if accuracy is poor (might be IP-based or weak GPS signal)
            if (!gpsLocked && accuracy > 100 && accuracy <= 500) {
                console.warn(`⚠️ Poor GPS accuracy: ${accuracy.toFixed(0)}m - This might be IP-based location`);
                const locIndicator = document.getElementById('location-indicator');
                if (locIndicator) {
            ...
            // Lock on GPS once accuracy is reasonable (true GPS is typically <50m, WiFi-assisted GPS is <100m)
            // Accept <100m accuracy as "good enough" for medical/clinic locating purposes
            if (!gpsLocked && accuracy <= 100) {
                gpsLocked = true;
                gpsLockedIn = true;
                ...
                if (accuracy > 50) accuracyLevel = 'WiFi-Assisted GPS (Good)';
                ...
                if (manualLocationUsed) {
                    ...
                    showNotification(`✅ GPS Locked! ${accuracyLevel} (±${accuracy.toFixed(0)}m) — Starting live tracking...`, 'success', 2500);
                }
                console.log('🎯 GPS LOCKED - Live tracking enabled');
                    ...
                    if (pois && pois.length > 0) {
                        ...
                        if (nearest) {
                            selectClinic(nearest.lat, nearest.lon, nearest.name, nearest.type, nearest.tags.addr || '', nearest);
                            showNotification(`🎯 Auto-routing to nearest clinic: ${nearest.name} (${minDistance.toFixed(1)} km)`, 'success', 3000);
                        }
                    }
            }

            // Update location (only if GPS locked or manual location used)
            if (gpsLocked || manualLocationUsed) {
                userLocationAccuracy = accuracy;
...
function updateUIAfterLocation(lat, lon) {
    ...
    const locIndicator = document.getElementById('location-indicator');
    if (locIndicator && isLiveTracking) {
...
function getAccurateLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // REJECT IP-BASED: Real GPS should be <100m accuracy
            // If accuracy > 100m, it's IP-based geolocation (not real GPS)
            if (accuracy > 100) {
                console.log(`⚠️ Rejected: Accuracy ${accuracy}m = IP-based, not real GPS`);
                ...
                showNotification('❌ Got IP-based location (not accurate GPS)\n\n✅ Solution:\n1. Go outdoors to open area\n2. Clear view of sky (no tall buildings)\n3. Wait 20-30 seconds for GPS lock\n4. Try again', 'error', 6000);
                return;
            }
            ...
            showNotification(`✅ Real GPS Found! ${accuracyStatus}\n±${accuracy.toFixed(0)}m accuracy`, 'success', 4000);
...
document.getElementById('stop-tracking')?.addEventListener('click', () => {
    ...
    const locIndicator = document.getElementById('location-indicator');
    if (locIndicator) locIndicator.innerHTML = '📍 Live tracking stopped';
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if ([401, 403, 500].includes(response.status)) {
                throw new Error('AI service not configured or temporarily unavailable.');
            }
            throw new Error(data?.error || 'AI service not configured or temporarily unavailable.');
        }
...
// Start GPS-only live tracking on page load
startAutoGeolocation();
...
console.log('🏥 Pediatrician Locator - GPS-Only Live Tracking Enabled');
